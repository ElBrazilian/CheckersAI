...
#define USER_INPUT_FILE "test.txt"
...
/****************************************************************/
/* le damier est ici une grille 10x10 (bordure extérieure)      */
...
long int last_update_timestamp = 0;

long int getFileCreationTime(char *path);
...
// return true si on peut sauter sur une position, ie manger le pion
...
// return true if any piece from the given color can "eat" an enemy piece
...
/************************************************************************/
// Actually "eat" a piece on the board, we are at (x, y) and the eaten piece is at (x+dx, x+dy)
...
void play_move(void)
{
    ...
    if (best_nb_jumps == 0) {
        if (print_move) printf("-%d", to); // on se deplace just : -
    } else {
        for (int i=0; i<best_nb_jumps; i++) {
            from  = best_move_positions[i];
            to    = best_move_positions[i+1];
            set_piece(between_position(from,to),EMPTY); // on jump : - x
...
void read_player_move(void)
{
    // wait for the file to be updated
    long int update_timestamp = getFileCreationTime(USER_INPUT_FILE);
    while (update_timestamp == last_update_timestamp){
        // wait 
        update_timestamp = getFileCreationTime(USER_INPUT_FILE);
        // fprintf(stderr, "still waiting..\n");
        sleep(1); // format is sleep(x); where x is # of seconds.
    }
    last_update_timestamp = update_timestamp;
    // fprintf(stderr, "not anymore..\n");

    FILE *user_file = NULL;
    user_file = fopen(USER_INPUT_FILE, "r");

    int from, to;
    best_nb_jumps = 0;
    fscanf(user_file, "%d",&from);
    best_move_positions[0] = from;
    char move_or_jump = 'a';
    fscanf(user_file, "%c",&move_or_jump);
    if (move_or_jump=='-') { // déplacement
        fscanf(user_file, "%d",&to);
        best_move_positions[1] = to;
    } else {                 // prise
        int fscanf_res = 0;
        do {
            best_nb_jumps += 1;
            fscanf(user_file, "%d",&best_move_positions[best_nb_jumps]);
            fscanf_res = fscanf(user_file, "%c",&move_or_jump);
        } while (move_or_jump=='x' && fscanf_res != EOF);
    }
...
int main(int argc, char *argv[])
{
    ...
    last_update_timestamp = getFileCreationTime(USER_INPUT_FILE);
    ...
    int turn = BLACK;
    while (!game_over()) {
        if (turn==IA_color) read_player_move();
        else read_move();
        play_move();
